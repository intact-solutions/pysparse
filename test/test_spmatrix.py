...
class LLMatSimpleTestCase(unittest.TestCase):
    ...
    def testCreate(self):
        self.assertTrue(self.A.shape == (self.n, self.n))
        self.assertTrue(self.A.nnz == 0)
        self.assertTrue(not self.A.issym)
        self.assertTrue(self.S.shape == (self.n, self.n))
        self.assertTrue(self.S.nnz == 0)
        self.assertTrue(self.S.issym)
    ...
    def testEntry(self):
        ...
        self.A[0,0] = 1.0
        self.S[0,0] = 1.0
        self.assertTrue(self.A[0,0] == 1.0)
        self.assertTrue(self.A.nnz == 1)
        self.assertTrue(self.S[0,0] == 1.0)
        self.assertTrue(self.S.nnz == 1)
        self.assertRaises(IndexError, assignUP)
        self.A[0,0] += 1.0
        self.assertTrue(self.A[0,0] == 2.0)
        self.assertTrue(self.A.nnz == 1)
        self.A[0,0] -= 2.0
        self.assertTrue(self.A[0,0] == 0.0)
        self.assertTrue(self.A.nnz == 0)
        ...
        self.assertRaises(IndexError, assignRight)
        self.assertRaises(IndexError, assignTop)
        self.assertRaises(IndexError, assignBottom)
        # negative indices
        ...
        for i in range(-10, 0):
            for j in range(-10, 0):
                self.assertTrue(I[i,j] == I[10+i,10+j])
...
class LLMatPoissonTestCase(unittest.TestCase):
    ...
    def testBasic(self):
        self.assertTrue(self.S.nnz == self.n*(3*self.n - 2))
        self.assertTrue(self.A.nnz == self.n*(5*self.n - 4))
        self.assertTrue(llmat_isEqual(self.A, self.A))
        self.assertTrue(llmat_isEqual(self.S, self.S))
        self.assertTrue(llmat_isEqual(self.A, self.S))
        self.assertTrue(llmat_isEqual(self.A, self.B))
    ...
    def testSubmatrix(self):
        ...
        for i in range(n):
            self.assertTrue(llmat_isEqual(self.A[n*i:n*(i+1),n*i:n*(i+1)], P))
            #self.failUnless(llmat_isEqual(self.S[n*i:n*(i+1),n*i:n*(i+1)], P))
            self.assertTrue(llmat_isEqual(self.A[n*i:n*(i+1),n*i:n*(i+1)], Psym))
        ...
        for i in range(n):
            ...
            self.assertTrue(llmat_isEqual(R[n*i:n*(i+1),n*i:n*(i+1)], P))
            R[n*i:n*(i+1),n*i:n*(i+1)] = Psym
            self.assertTrue(llmat_isEqual(R[n*i:n*(i+1),n*i:n*(i+1)], Psym))
        # store and get off-diagonal blocks
        for i in range(n-1):
            R[n*i:n*(i+1),n*(i+1):n*(i+2)] = P
            self.assertTrue(llmat_isEqual(R[n*i:n*(i+1),n*(i+1):n*(i+2)], P))
            R[n*i:n*(i+1),n*(i+1):n*(i+2)] = Psym
            self.assertTrue(llmat_isEqual(R[n*i:n*(i+1),n*(i+1):n*(i+2)], Psym))
        ...
        for i in range(n-1):
            ...
            self.assertTrue(llmat_isEqual(R[n*(i+1):n*(i+2),n*i:n*(i+1)], P))
            R[n*(i+1):n*(i+2),n*i:n*(i+1)] = Psym
            self.assertTrue(llmat_isEqual(R[n*(i+1):n*(i+2),n*i:n*(i+1)], Psym))
...
class LLMatDeleteRowColsTestCase(unittest.TestCase):
    ...
    def testDeleteRowColsSym(self):
        ...
        self.assertTrue(llmat_isEqual(self.S, self.P))
        
    def testDeleteRowColsGen(self):
        self.A.delete_rowcols(self.mask)
        self.assertTrue(llmat_isEqual(self.A, self.P))
    ...
    def testDeleteRowColsGen2StepOff(self):
        ...
        self.assertTrue(llmat_isEqual(self.A, self.I))
...
class LLMatNorm(unittest.TestCase):
    ...
    def testNormGeneral(self):
        ...
        self.assertTrue(A.norm('1') == 8)
        self.assertTrue(A.norm('inf') == 8)
        self.assertTrue(poisson.poisson1d(3).norm('fro') == 4)
            
    def testNormSymmetric(self):
        A = spmatrix.ll_mat_sym(4)
        A[0,0] = 1; A[1,1] = 2; A[2,2] = 3; A[3,3] = 4;
        A[1,0] = 3; A[2,0] = 2; A[3,0] = 2; 
        self.assertTrue(A.norm('fro') == 8)
      ...
      def testRandomMat(self):
          ...
          for i in range(100):
              ...
              for s in range(10):
                  ...
                  B.matvec(x, t)
                  A.matvec(t, y2)
                  self.assertTrue(math.sqrt(numpy.dot(y1 - y2, y1 - y2)) < eps * n*m*k)
